...
#include "filesystem/pf_internal.h"
...
RC PF_Manager::CreateFile(const char *fileName)
{
    ...
    //	if ((fd = open(fileName, O_BINARY | O_CREAT | O_EXCL | O_WRONLY,
    // CREATION_MASK)) < 0)
    //      return (PF_UNIX);
...
//       to point to the file data in the file foreign_table, and to point to
...
RC PF_Manager::OpenFile(const char *fileName, PF_FileHandle &fileHandle)
{
    ...
    {
        int numBytes = read(
            fileHandle.unixfd, (char *) &fileHandle.hdr, sizeof(PF_FileHdr));
