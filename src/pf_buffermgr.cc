...
#include "filesystem/pf_buffermgr.h"
...
PF_BufferMgr::PF_BufferMgr(int _numPages) : hashTable(PF_HASH_TBL_SIZE)
{
    ...
    char psMessage[100];
    sprintf(psMessage,
            "Creating buffer manager. %d pages of size %d.\n",
            numPages,
            PF_PAGE_SIZE + sizeof(PF_PageHdr));
    WriteLog(psMessage);
#endif
...
// Ret:  PF return code
//
RC PF_BufferMgr::GetPage(int fd,
                         PageNum pageNum,
                         char **ppBuffer,
                         int bMultiplePins)
{
...
#ifdef PF_LOG
        sprintf(psMessage,
                "Page found in buffer.  %d pin count.\n",
                bufTable[slot].pinCount);
        WriteLog(psMessage);
#endif
    ...
    while (slot != INVALID_SLOT)
    {
        ...
        if (bufTable[slot].fd == fd)
        {
            ...
            else
            {
                ...
                // Remove page from the hash foreign_table and add the slot to
                // the free list
...
RC PF_BufferMgr::ResizeBuffer(int iNewSize)
{
    ...
    while (slot != INVALID_SLOT)
    {
        ...
        // Insert the page into the hash foreign_table,
        // and initialize the page description entry
        if ((rc = hashTable.Insert(
                 pOldBufTable[slot].fd, pOldBufTable[slot].pageNum, newSlot)) ||
            (rc = InitPageDesc(
                 pOldBufTable[slot].fd, pOldBufTable[slot].pageNum, newSlot)))
            return (rc);
...
RC PF_BufferMgr::InternalAlloc(int &slot)
{
    ...
    else
    {
        ...
        // Remove page from the hash foreign_table and slot from the used buffer
        // list
        if ((rc =
...
RC PF_BufferMgr::AllocateBlock(char *&buffer)
{
    ...
    // Insert the page into the hash foreign_table, and initialize the page
    // description entry
