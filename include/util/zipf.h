...
[[maybe_unused]] static void mehcached_zipf_init_copy(
...
[[maybe_unused]] static void mehcached_zipf_change_n(
    struct zipf_gen_state *state, uint64_t n)
{
    state->n = n;
}
...
static uint64_t mehcached_zipf_next(struct zipf_gen_state *state)
{
    if (state->last_n != state->n)
    {
        ...
        {
            state->zetan = mehcached_zeta(
                state->last_n, state->zetan, state->n, state->theta);
            state->eta =
...
[[maybe_unused]] static void mehcached_test_zipf(double theta)
{
    double zetan = 0.;
