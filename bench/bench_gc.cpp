...
DEFINE_uint32(gc_ratio, 1, "[0-100] the percentage of gc");
...
void bench_kv(IEngine::Pointer engine)
{
    ...
    for (size_t i = 0; i < FLAGS_thread_nr; ++i)
    {
        ...
        double gc_ratio = 1.0 * FLAGS_gc_ratio / 100;
        double ins_ratio = (1 - read_ratio - gc_ratio) / 2;
