...
DEFINE_uint32(read_ratio, 75, "[0-100] the percentage of read");
DEFINE_uint32(visit_ratio, 5, "[0-100] the percentage of visit");
...
void explain()
{
    LOG(INFO) << "[explain] test_nr: " << FLAGS_test_nr
              ...
              << ", visit_ratio[0-100]: " << FLAGS_visit_ratio
...
void bench_kv(IEngine::Pointer engine)
{
    ...
    for (size_t i = 0; i < FLAGS_thread_nr; ++i)
    {
        ...
        double visit_ratio = 1.0 * FLAGS_visit_ratio / 100;
        double ins_ratio = (1 - read_ratio - visit_ratio) / 2;
    ...
    for (size_t i = 0; i < FLAGS_thread_nr; ++i)
    {
        threads.emplace_back(
            [&task_nr, &test_case = test_cases[i], engine]()
            {
                ...
                {
                    auto current_task_nr = std::min(
                        int64_t(remain_task_nr), int64_t(FLAGS_execute_batch));

                    executor.execute(test_case, cur_idx, current_task_nr);

                    task_nr.fetch_sub(current_task_nr,
